...
module.exports = ({strapi}) => {
  const connect = (provider, ownerPlugin, query) => {
    const access_token = query.access_token || query.code || query.oauth_token

    return new Promise((resolve, reject) => {
      if (!access_token && !query.providerArgs) {
        return reject([null, {message: 'No access_token.'}])
      }
      const callback = async (err, profile) => {
        if (err) {
        ...
        try {
          ...
          const params = _.assign(profile, {
            provider: provider,
            role: profile.role || defaultRole.id,
            confirmed: true,
          })
      ...
      // Get the profile.
      if (ownerPlugin === pluginId) {
        getProfile(provider, query, callback)
      } else {
        try {
